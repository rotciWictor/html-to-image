...
class TextTranslator {
  ...
  extractContentBlocks(html) {
    ...
    const blockTagsRegex = /<(div|p|ul|ol|li|h[1-6]|table|blockquote|section|article|header|footer)\b/i;
    const candidates = [];

    $('*').each((i, el) => {
      if (['html', 'head', 'body', 'script', 'style', 'title', 'meta', 'link'].includes(el.name)) return;
      ...
      // Ignora blocos que são apenas handles de redes sociais (ex: @era.outro.meu.plano)
      if (/^@[a-zA-Z0-9_.-]+$/.test(text)) return;
      
      // Se o elemento possui apenas texto ou tags inline (b, i, span), ele é um candidato
      if (!blockTagsRegex.test(inner)) {
        candidates.push(el);
      }
    });

    // Remover candidatos que estão dentro de outros candidatos (para não quebrar spans aninhados)
    ...
    return { maskedHtml: $.html(), segments };
  }
  ...
  async translateViaJson(texts, targetLang, globalContext) {
    ...
    for (let i = 0; i < texts.length; i += BATCH_SIZE) {
      ...
      // O CacheManager já usa MD5, então podemos passar o texto inteiro do contexto
      // para garantir que se o contexto da história mudar uma vírgula, a tradução será reavaliada.
      const cacheKey = this.cache.generateKey(JSON.stringify(batch) + targetLang + globalContext + 'v17');
      ...
      console.log(`   📨 [Linguista] Traduzindo lote ${Math.ceil((i+1)/BATCH_SIZE)} com contexto...`);

      // PROMPT COM CONTEXTO DA HISTÓRIA
      const prompt = `You are an expert literary translator and localization specialist.
      
STORY CONTEXT (Use this to understand tone, context, and vocabulary):
"""
${globalContext}
"""
...
SPECIFIC RULES:
1. DO NOT translate literally. Adapt the text so it sounds natural, engaging, and culturally appropriate in ${tLang}. Keep the exact same tone and humor.
2. Use the CONTEXT above to disambiguate words (e.g., if the story is about fire, "Brasa" -> "Ember").
3. Return ONLY a valid JSON array of strings.
4. Translate everything including uppercase words, preserving the original intent.
5. Keep numbers exactly as they are.
...
OUTPUT JSON:`;
      ...
      try {
        const response = await this.translateAdapter.generateContent(prompt, { temperature: 0.1, format: 'json' });
        ...
        try {
          ...
          console.warn(`   ⚠️ [Linguista] Falha ao parsear JSON. Retornando texto original. Output bruto da IA:\n${response}`);
          resultBatch = batch; 
        }

        if (!Array.isArray(resultBatch) || resultBatch.length !== batch.length) {
           console.warn(`   ⚠️ [Linguista] Tamanho do array não bate. Esperado: ${batch.length}, Recebido: ${Array.isArray(resultBatch) ? resultBatch.length : 'não é array'}.`);
           resultBatch = batch;
        }
...
RESULT:`;
  ...
  adjustAssetPaths(htmlContent, targetDir) {
    const assetsRoot = require('path').resolve(process.cwd(), 'assets');
    const relPath = require('path').relative(targetDir, assetsRoot).replace(/\\/g, '/');
    return htmlContent.replace(/((?:src|href)=["']|url\(['"]?)(?:\/|\.\.\/|\.\/)*(assets\/[^"'\)]+)/gi, (match, prefix, assetPath) => {
      return `${prefix}${relPath}/${assetPath.replace(/^assets\//i, '')}`;
    });
  }
  ...
  saveTranslation(originalPath, content, lang, options) {
    ...
    const fixedContent = this.adjustAssetPaths(content, targetDir);
