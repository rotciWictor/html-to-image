...
class ArchiveProcessor {
  ...
  async extractZip(archivePath, extractPath) {
    ...
    try {
      // Extrai todos os arquivos. A API async retorna a quantidade extraída em alguns ambientes
      const extractedCount = await zip.extract(null, extractPath);
      if (typeof extractedCount === 'number') {
        console.log(`✅ Extração concluída: ${extractedCount} arquivo(s) extraído(s)`);
      } else {
   ...
   */
  async processArchive(archivePath, extractPath = null) {
    const tempDir = extractPath || path.join(path.dirname(archivePath), 'temp_' + Date.now());
    ...
    try {
      ...
      if (!extractPath && fs.existsSync(tempDir)) {
