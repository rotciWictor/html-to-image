...
class ProcessingOrchestrator {
  ...
  async processFolder(folder, options) {
    ...
    if (options.pdf && results.length > 0) {
      const successfulImages = results.filter(r => r.success).map(r => r.outputFile);
      if (successfulImages.length > 0) {
        const PdfGenerator = require('./PdfGenerator');
        const pdfGen = new PdfGenerator();
        try {
           const finalOutputDir = options.outDir || (pathStats.isFile() ? path.dirname(folder) : folder);
           await pdfGen.generateFromImages(successfulImages, finalOutputDir);
        } catch (err) {
    ...
    try {
      ...
      if (options.pdf && results.length > 0) {
        ...
        if (successfulImages.length > 0) {
          ...
          const pdfGen = new PdfGenerator();
          try {
             const finalOutputDir = options.outputDir || path.dirname(htmlPaths[0]);
             await pdfGen.generateFromImages(successfulImages, finalOutputDir);
          } catch (err) {
             console.error('❌ Erro ao gerar PDF:', err.message);
          }
