...
class TemplateGenerator {
  constructor() {
    this.templates = {
      ...
      stories: this.getStoriesTemplate.bind(this),
      ppt: this.getPowerPointTemplate.bind(this),
      generic: this.getGenericTemplate.bind(this)
    };
  }

  generateTemplates(type, count = 1, outputDir = './work/htmls', options = {}) {
    const {
      width = 1200,
      height = 800,
      format = 'png',
  ...
  getInstagramTemplate(config) {
    const { width = 1080, height = 1440, format = 'png', quality = 90, background = 'transparent', slideNumber = 1, totalSlides = 1 } = config;
...
<body>
  ...
  getStoriesTemplate(config) {
    const { width = 1920, height = 1080, format = 'png', quality = 90, background = 'transparent', slideNumber = 1, totalSlides = 1 } = config;
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Stories Slide ${slideNumber}</title>
