...
class InteractiveWizard {
  ...
  async selectProject() {
    ...
    const projectChoices = [
      { title: '⚡ Modo Rápido (Novo Projeto Automático)', value: '_auto' },
      { title: '➕ Criar Novo Projeto (Nome Customizado)', value: '_new' },
    ...
    if (response.action === 'ai') {
      ...
      if (aiResponse.provider === 'gemini') {
        const { geminiModel } = await prompts({
          ...
          choices: [
            { title: 'Gemini 2.5 Flash (Padrão, Rápido, Gratuito)', value: 'gemini-2.5-flash' },
            { title: 'Gemini 2.5 Pro (Mais inteligente, Lento)', value: 'gemini-2.5-pro' },
            { title: 'Gemini 1.5 Pro (Legado)', value: 'gemini-1.5-pro' },
            { title: 'Gemini 1.5 Flash (Legado)', value: 'gemini-1.5-flash' }
          ]
        });
