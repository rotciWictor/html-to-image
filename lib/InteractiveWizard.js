...
class InteractiveWizard {
  ...
  async selectProject() {
    ...
    this.assetsFolder = path.join(this.projectRoot, 'assets');
  ...
  async run() {
    ...
    if (response.action === 'ai') {
      const aiResponse = await prompts([
        {
          type: 'select',
          name: 'provider',
          message: 'Qual provedor de IA de texto (Coder) você deseja usar?',
          choices: [
        ...
        },
        {
          type: 'select',
          name: 'imageProvider',
          message: 'Qual gerador de imagens você deseja usar?',
          choices: [
            { title: 'Pollinations.ai (Hub de Modelos, via Chave ou Grátis)', value: 'pollinations' },
            { title: 'Hugging Face Serverless (Gratuito, requer HF_API_KEY)', value: 'huggingface' },
            { title: 'Google Gemini Imagen (Requer GEMINI_API_KEY)', value: 'gemini' },
            { title: 'ComfyUI (Local, requer Servidor ComfyUI rodando)', value: 'comfyui' }
          ],
          initial: 0
        },
        {
          type: prev => prev === 'pollinations' ? 'select' : null,
          name: 'pollinationsModel',
          message: 'Qual modelo do Pollinations.ai você quer usar?',
          choices: [
            { title: 'Flux Schnell (Padrão, super rápido)', value: 'flux' },
            { title: 'FLUX.2 Klein 4B (Alpha, nova geração)', value: 'flux-realism' }, // usando alias comum no hub deles
            { title: 'Z-Image Turbo (Focado em velocidade)', value: 'turbo' },
            { title: 'Grok Imagine', value: 'grok' },
            { title: 'Flux Anime', value: 'flux-anime' },
            { title: 'Flux 3D', value: 'flux-3d' }
          ],
          initial: 0
        },
        {
      ...
      if (aiResponse.imageProvider) {
        process.env.IMAGE_PROVIDER = aiResponse.imageProvider;
      }
      
      if (aiResponse.pollinationsModel) {
        process.env.POLLINATIONS_MODEL = aiResponse.pollinationsModel;
      }
      ...
      while (true) {
        ...
        try {
          ...
          console.log(`\n💎 Plano da Gema:`);
          console.log('\x1b[36m%s\x1b[0m', reply);
          ...
          const nextAction = await prompts({
            ...
            choices: [
              { title: '✏️  Pedir alterações no plano', value: 'feedback' },
              { title: '✅ Aprovar plano e gerar imagens/HTMLs', value: 'approve' },
          ...
          if (nextAction.action === 'feedback') {
            const feedbackInput = await prompts({
              type: 'text',
              name: 'text',
              message: 'Descreva as alterações que deseja no plano:'
            });
          ...
          if (nextAction.action === 'approve') {
             // Fase 2: Geração de Imagens
             let finalImagesContext = '';
             if (imageService) {
                console.log(`\n📸 Gerando imagens necessárias para o layout...`);
                const imgMatches = reply.match(/\[GERAR_IMAGEM:\s*(.+?)\]/gi) || [];
                const generatedImagesInfo = [];
                for (const match of imgMatches) {
                  ...
                  try {
                    const imgPath = await imageService.generate(imgPrompt, { width, height, assetsDir: this.assetsFolder });
                    const finalRelPath = `../assets/${path.basename(imgPath)}`;
                    generatedImagesInfo.push(`- Caminho: ${finalRelPath} (Criada com as dimensões: ${width}x${height})`);
                    console.log(`✅ [Imagem Gerada]: ${finalRelPath}`);
                  } catch (imgErr) {
                    console.error(`❌ Falha ao gerar imagem: ${imgErr.message}`);
                    generatedImagesInfo.push(`- Caminho: error-generating.png (Erro de geração)`);
                  }
                }
                
                if (generatedImagesInfo.length > 0) {
                  finalImagesContext = `[SISTEMA] As imagens aprovadas no plano foram geradas fisicamente no disco. Segue a lista de imagens para uso no código:\n${generatedImagesInfo.join('\n')}`;
                } else {
                  finalImagesContext = `[SISTEMA] Nenhuma tag de imagem foi detectada no plano.`;
                }
             }

             // Fase 3: Geração de HTML
             console.log(`\n💻 Escrevendo o código HTML... (via ${aiResponse.provider})`);
             const coderPromptPath = path.join(process.cwd(), 'config', 'prompts', 'coder.txt');
             let coderPrompt = fs.existsSync(coderPromptPath) ? fs.readFileSync(coderPromptPath, 'utf8') : 'Você é o desenvolvedor HTML.';
             
             let coderMsg = `Plano aprovado! O plano foi o último que conversamos. ${finalImagesContext}\nAgora, assuma seu papel de Desenvolvedor Frontend e gere APENAS os blocos \`\`\`html para CADA slide, sem explicações extras. Lembre-se de não inventar imagens falsas nem gerar tags falsas, apenas use EXATAMENTE os caminhos reais que passei acima, ou cores sólidas/gradientes.`;
             
             let isCoding = true;
             while (isCoding) {
                 const coderReply = await chat.sendMessageWithSystemPrompt(coderMsg, coderPrompt, true);
                 
                 const htmlMatches = coderReply.match(/```html([\s\S]*?)```/gi) || [];
                 if (htmlMatches.length > 0) {
                    if (!fs.existsSync(this.workFolder)) fs.mkdirSync(this.workFolder, { recursive: true });
                    
                    // Limpar arquivos da iteração anterior para não acumular lixo
                    sessionFiles.forEach(f => { if(fs.existsSync(f)) fs.unlinkSync(f); });
                    sessionFiles.length = 0; // reset

                    htmlMatches.forEach((block, index) => {
                      let htmlCode = block.replace(/```html|```/gi, '').trim();
                      // Sobrescreve sempre o mesmo arquivo de slide para não criar múltiplos
                      const filename = `gema-design-slide-${index}.html`;
                    ...
                    console.log(`\n👁️  Preview gerado: ${previewUrl}`);
                    try {
                      const { exec } = require('child_process');
                      exec(`start "" "${previewUrl}"`);
                    } catch (e) { /* fallback */ }
                    
                    const postGenAction = await prompts({
                      type: 'select',
                      name: 'action',
                      message: 'O código HTML foi gerado e salvo. O que deseja fazer agora?',
                      choices: [
                        { title: '✏️  Pedir correções no HTML (escrever feedback)', value: 'feedback' },
                        { title: '✅ Converter para imagem final', value: 'convert' },
                        { title: '❌ Sair sem converter', value: 'exit' }
                      ]
                    });

                    if (postGenAction.action === 'feedback') {
                        const feedbackInput = await prompts({
                          type: 'text',
                          name: 'text',
                          message: 'Descreva as correções que deseja no código HTML:'
                        });
                        if (!feedbackInput.text) { isCoding = false; break; }
                        coderMsg = `O usuário revisou o HTML gerado e pediu a seguinte correção:\n"${feedbackInput.text}"\nRefaça todos os blocos de HTML ajustando conforme solicitado.`;
                        console.log(`\n💻 Reescrevendo o código HTML com as correções...`);
                        continue;
                    } else if (postGenAction.action === 'convert') {
                      console.log('\n✅ Iniciando conversão para imagem...');
                      await previewServer.close();
                      if (sessionFiles.length > 0) {
                        console.log(`🎯 Convertendo os ${sessionFiles.length} HTML(s)...`);
                        const options = await this.promptForPDF({ outputDir: this.outputFolder });
                        return await this.orchestrator.convertFiles(sessionFiles, options);
                      }
                      isCoding = false;
                    } else {
                      isCoding = false;
                    ...
                    console.log('\n❌ Nenhum HTML foi retornado pelo assistente na fase de codificação.');
                    isCoding = false;
                 }
             }
  ...
  async promptForPDF(options) {
    const response = await prompts({
      type: 'confirm',
      name: 'pdf',
      message: 'Deseja também gerar um PDF com as imagens finais? (Útil para apresentações)',
      initial: false
    });
    if (response.pdf) {
      options.pdf = true;
