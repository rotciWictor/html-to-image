  ...
  async run() {
    ...
    if (response.action === 'translate' || response.action === 'translate_specific') {
      ...
      if (response.action === 'translate') {
        ...
        // Pergunta se o usuário também quer converter os arquivos traduzidos em imagens agora
        const convResponse = await prompts({
          type: 'confirm',
          name: 'convert',
          message: 'Tradução concluída! Deseja gerar as imagens (PNG) desses arquivos agora?',
          initial: true
        });

        if (convResponse.convert) {
          console.log(`\n📸 Gerando imagens para os arquivos traduzidos...`);
          // Passa config em branco para usar o padrão da pasta
          return await this.orchestrator.convertFiles(translatedFiles, {});
        } else {
          console.log('✔️  Tradução salva em HTML sem gerar imagens.');
          process.exit(0);
        }
      }
    }
