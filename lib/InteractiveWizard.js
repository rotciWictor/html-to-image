  ...
  async run() {
    ...
    const response = await prompts({
      ...
      choices: [
        ...
        { 
          title: '🌍 Traduzir arquivos HTML (Multi-Agente)', 
          value: 'translate',
          description: 'Traduz os textos dos seus HTMLs para outro idioma mantendo o código'
        },
        { 
    ...
    if (response.action === 'translate') {
      const transResponse = await prompts([
        {
          ...
          initial: 'en',
          validate: text => text.length >= 2 ? true : 'Digite um código válido de idioma (ex: en)'
        },
        {
          type: 'select',
          name: 'provider',
          message: 'Qual provedor de IA você deseja usar para a tradução?',
          choices: [
      ...
      console.log(`\n🌍 Iniciando tradução para '${transResponse.targetLang}' usando ${transResponse.provider}...`);
