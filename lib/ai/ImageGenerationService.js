...
class ImageGenerationService {
  ...
  _createAdapter() {
    ...
    if (this.provider === 'huggingface') {
      const HuggingFaceImageAdapter = require('./HuggingFaceImageAdapter');
      console.log(`🎨 Gerador de Imagens: Hugging Face (Serverless Inference)`);
      return new HuggingFaceImageAdapter();
    }

    // Default Fallback: Pollinations (Gratuito, sem token)
    const PollinationsImageAdapter = require('./PollinationsImageAdapter');
    console.log(`🎨 Gerador de Imagens: Pollinations.ai (API Aberta Gratuita)`);
  ...
  async generate(prompt, options = {}) {
    ...
    const targetDir = options.assetsDir || this.assetsDir;
    ...
    try {
      ...
      let imageBuffer;
      try {
        imageBuffer = await this.adapter.generateImage(prompt, {
          width: options.width || 1024,
          height: options.height || 1024,
          ...options
        });
      } catch (adapterError) {
        if (this.provider === 'pollinations') throw adapterError; // Se já é pollinations, não faz fallback pra ele mesmo
        console.error(`❌ [Imagen] Provedor principal (${this.provider}) falhou: ${adapterError.message}`);
        console.log(`🔄 [Imagen] Tentando Fallback de Segurança (Pollinations.ai)...`);
        
        const PollinationsImageAdapter = require('./PollinationsImageAdapter');
        const fallbackAdapter = new PollinationsImageAdapter();
        imageBuffer = await fallbackAdapter.generateImage(prompt, {
          width: options.width || 1024,
          height: options.height || 1024,
          ...options
        });
      ...
      // Retornar caminho absoluto para que o orquestrador resolva como quiser
      return outputPath;

    } catch (error) {
      console.error(`❌ [Imagen] Falha Crítica ao gerar imagem: ${error.message}`);
      throw error;
    }
  }
