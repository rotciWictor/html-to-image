  ...
  async startAssetServer() {
    if (!this.server) {
      ...
      app.get('/preview', (req, res) => {
        ...
        const htmlDir = path.dirname(absolutePath);
        const cwd = process.cwd();
        
        // Resolver caminhos relativos de assets para URLs do servidor local (src, href)
        html = html.replace(/(src|href)\s*=\s*(["'])((?:\.\.\/|\.\/|\/)*)(assets\/[^"']+)\2/gi, (match, attr, quote, prefix, assetPath) => {
          const originalRef = prefix + assetPath;
          const assetAbsolute = path.resolve(htmlDir, originalRef);
          const rootRelative = path.relative(cwd, assetAbsolute).replace(/\\/g, '/');
          return `${attr}=${quote}http://localhost:${this.serverPort}/assets/${rootRelative}${quote}`;
        });
        
        // Resolver caminhos relativos dentro de CSS url()
        html = html.replace(/url\(\s*(["'])?((?:\.\.\/|\.\/|\/)*)(assets\/[^"')]+)\1?\s*\)/gi, (match, quote, prefix, assetPath) => {
          const q = quote || '"';
          const originalRef = prefix + assetPath;
          const assetAbsolute = path.resolve(htmlDir, originalRef);
          const rootRelative = path.relative(cwd, assetAbsolute).replace(/\\/g, '/');
          return `url(${q}http://localhost:${this.serverPort}/assets/${rootRelative}${q})`;
        });
...
<body>
  ...
  processAssetPaths(htmlContent, htmlFilePath) {
    ...
    // Processar TODAS as referências url() em CSS (inline ou em blocos <style>)
    // Isso captura background-image, background shorthand, list-style-image, cursor, etc.
    // Inclui casos compostos como: background-image: linear-gradient(...), url('...');
    processed = processed.replace(/url\(\s*(["']?)([^"')]+)\1\s*\)/gi, (match, quote, assetPath) => {
      if (assetPath) {
        console.log(`\n   🎨 Processando CSS url(): ${assetPath}`);
        const newMatch = processAssetPath(assetPath, match);
        return newMatch;
      }
      return match;
