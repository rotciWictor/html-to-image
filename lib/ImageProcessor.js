...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    body { margin: 0; font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, Noto Sans, Helvetica Neue, Arial; background: var(--bg); color: var(--text); display: grid; grid-template-columns: var(--panel-width) 1fr; height: 100vh; }
    ...
    li { margin: 4px 0; }
    a.item { display: flex; gap: 8px; padding: 8px 10px; color: var(--text); border-radius: 6px; border: 1px solid transparent; text-decoration: none; }
    ...
    .frame-wrap { width: 1080px; height: 1080px; background: #fff; box-shadow: 0 10px 30px rgba(0,0,0,.35); transform-origin: top left; overflow: hidden; transition: transform 0.2s ease-out; }
  ...
  <script>
    ...
    function load(index){
      ...
      // Carrega o iframe usando a nossa super rota /preview que injeta os assets locais
      const url = '/preview?file=' + encodeURIComponent(file);
      byId('slideInner').src = url;
    ...
    function fitScale(){ const { w, h } = viewerSize(); const zw = w / baseW; const zh = h / baseH; return Math.max(0.1, Math.min(2, Math.min(zw, zh) - 0.05)); }
