...
<head>
    ...
    <title>Preview da Gema 💎</title>
    <style>
        body { margin: 0; padding: 0; background-color: #1a1a1a; color: #fff; font-family: 'Inter', sans-serif; height: 100vh; display: flex; flex-direction: column; }
        header { display: flex; justify-content: space-between; align-items: center; padding: 10px 20px; background: #2d2d2d; border-bottom: 1px solid #444; }
        .controls { display: flex; gap: 10px; align-items: center; }
        button { background: #4caf50; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer; font-weight: bold; }
        button:hover { background: #45a049; }
        button:disabled { background: #555; color: #888; cursor: not-allowed; }
        .counter { font-size: 14px; font-weight: bold; }
        .viewer { flex: 1; display: flex; justify-content: center; align-items: center; padding: 20px; overflow: hidden; }
        iframe { border: none; background: white; box-shadow: 0 10px 30px rgba(0,0,0,0.5); max-height: 100%; max-width: 100%; transition: opacity 0.2s; }
    </style>
</head>
<body>
    <header>
        <div>💎 Gema Preview</div>
        <div class="controls">
            <button id="prevBtn" onclick="change(-1)">◀ Anterior</button>
            <span class="counter"><span id="currentIdx">1</span> / ${files.length}</span>
            <button id="nextBtn" onclick="change(1)">Próximo ▶</button>
        </div>
    </header>
    <div class="viewer">
        <!-- O iframe carrega a rota /preview individual de cada arquivo -->
        <iframe id="frame" src="/preview?file=${encodeURIComponent(files[0])}" width="1080" height="1080"></iframe>
    </div>
    ...
    <script>
        ...
        function updateUI() {
            ...
            // Ocultar iframe levemente durante o carregamento
            frame.style.opacity = '0.5';
            frame.src = '/preview?file=' + encodeURIComponent(files[currentIndex]);
