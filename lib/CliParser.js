...
class CliParser {
  ...
  setupCommands() {
    this.program
      ...
      .version('1.0.0')
      .argument('[path]', 'Pasta com arquivos HTML ou arquivo HTML específico', 'work/htmls')
  ...
  parse(args = process.argv) {
    ...
    const inputPath = this.program.args[0] || 'work/htmls';

    // Resolver caminho
    let resolvedPath = path.resolve(inputPath);
    
    // Se o caminho não existe e não é um caminho absoluto ou relativo com diretório,
    // tentar procurar em work/htmls/
    if (!fs.existsSync(resolvedPath)) {
      // Se o inputPath não contém separadores de diretório, pode ser apenas um nome de arquivo
      if (!inputPath.includes(path.sep) && !inputPath.includes('/') && !path.isAbsolute(inputPath)) {
        const workHtmlsPath = path.join(process.cwd(), 'work', 'htmls', inputPath);
        const workHtmlsPathWithExt = inputPath.endsWith('.html') 
          ? workHtmlsPath 
          : workHtmlsPath + '.html';
        
        // Tentar com e sem extensão .html
        if (fs.existsSync(workHtmlsPathWithExt)) {
          resolvedPath = workHtmlsPathWithExt;
        } else if (fs.existsSync(workHtmlsPath)) {
          resolvedPath = workHtmlsPath;
        }
        // Se ainda não encontrou, também tentar como pasta
        else {
          const workHtmlsDir = path.join(process.cwd(), 'work', 'htmls');
          if (fs.existsSync(workHtmlsDir)) {
            // Verificar se existe um arquivo com esse nome na pasta work/htmls
            try {
              const files = fs.readdirSync(workHtmlsDir);
              const matchingFile = files.find(f => 
                f === inputPath || 
                f === inputPath + '.html' ||
                f.replace('.html', '') === inputPath
              );
              if (matchingFile) {
                resolvedPath = path.join(workHtmlsDir, matchingFile);
              }
            } catch (err) {
              // Ignorar erro, usar caminho original
            }
          }
        }
      }
